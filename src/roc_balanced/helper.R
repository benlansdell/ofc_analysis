    ...
    for (evt in events_of_interest) {
      for (dss in per_cell_ds) {
        if (length(unique(dss[,..evt])) > 1) {
          ...
          print(paste(evt, "not found, continuing."))
          skip_this = TRUE
        }
      }
    }
...
compute_all_rocs_balanced = function(path, recordings, events_of_interest, N, alpha, balanced = TRUE) { 
  #' For a set of recordings, run through entire ROC analysis, balancing datasets when possible (e.g. over-sampling minority trials)
  #' 
  #' @description Preprocess, compute ROC AUC for each cell in a set of recordings, and determine
  #' its statistical significance. 
  #' 
  ...
  for (fn in recordings) {
    ...
    event_counts <- ds[, .N, by = c('Animal', 'event', 'Trial Number')]
    l_counts <- event_counts[event == 'lspout_first',.N]
    r_counts <- event_counts[event == 'rspout_first',.N]
    
    other_events = c('In Start',
                     'In Middle',
                     'In Reward')
    
    right_events <- c('middle_right',
                      'In Right decision',
                      'In Right',
                      'rspout_first',
                      'rspout_exit',
    ...
    other_rows <- ds[event %in% other_events]
    right_rows <- ds[event %in% right_events]
    left_rows <- ds[event %in% left_events]
    
    if (nrow(right_rows) == 0 | nrow(left_rows) == 0) {
      print("Too unbalanced -- skipping")
      next
    }
    
    if (balanced == TRUE) {
      if (l_counts < r_counts - 2) {
        #Resample left events
        left_rows <- left_rows[sample(nrow(left_rows), nrow(right_rows), replace=TRUE), ]
      } else if (r_counts < l_counts - 2) {
        right_rows <- right_rows[sample(nrow(right_rows), nrow(left_rows), replace=TRUE), ]
      }
    }
          
    #Concat back together
    ds_balanced <- rbindlist(list(other_rows, right_rows, left_rows))
  
    
    roc_curves = hash()
    per_cell_ds = split(ds_balanced, f = ds_balanced$CellID)
    cells = dt[(`Trial Number` == 1) & (frame == 1), CellID]
    matches = dt[(`Trial Number` == 1) & (frame == 1), MatchID]
    animal = dt[1, Animal]
    session = dt[1, Session]
